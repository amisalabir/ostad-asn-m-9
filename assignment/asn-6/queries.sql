-- Table and Data creation queries
...
SELECT customers.*, COUNT(orders.id) AS total_order_placed
FROM customers
INNER JOIN orders ON customers.id = orders.customer_id
GROUP BY orders.customer_id ORDER BY total_order_placed DESC;
...
SELECT o.id,p.name, i.quantity, o.total_amount
FROM products p
...
SELECT c.name AS category_name,SUM(i.quantity*i.unit_price) AS revenue
FROM categories c
